...
export default function Photosuploader({ addedphotos, onChange }) {
  ...
  function selectasmainphoto(e,filename){
    e.preventDefault();
    const addedphotoswithoutselecte=addedphotos.filter(photo => photo !== filename)
    const newaddedphotos=[filename,...addedphotoswithoutselecte]
  ...
  return (
    <>
      ...
      <div className=" mt-2 grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
        ...
        <label className=" h-32cursor-pointer  flex gap-1 justify-center border bg-transparent rounded-2xl p-8 text-2xl text-gray-600">
          <input
            type="file"
            multiple
            className="hidden"
